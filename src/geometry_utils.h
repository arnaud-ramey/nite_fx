...
inline std::string pose_to_string(const _Pose3 & pose) {
  std::ostringstream ans;
  ans << "pos:" << geometry_utils::printP(pose.position)
      << ", orien:" << geometry_utils::printP4(pose.orientation);
  return ans.str();
}
...
inline std::string pose_stamped_to_string(const _Pose3Stamped & pose) {
  ...
  ans << "pose: {"
      ...
      << "pos:" << geometry_utils::printP(pose.pose.position)
      << ", orien:" << geometry_utils::printP4(pose.pose.orientation)
      << " }";
  return ans.str();
}
...
static inline Vector3 project_vector_on_plan(const Vector3 & vector,
  ...
  //maggieDebug2("vector:%s", printP(vector).c_str());
  //    maggieDebug2("normal_to_plan:%s", printP(normal_to_plan).c_str());
  //    maggieDebug2("module:%f", vector.dot(normal_to_plan));
  //    maggieDebug2("vector_normal_comp:%s", printP(vector_normal_comp).c_str());
...
static inline double angle_between_vector_and_plan(const Vector3 & vector,
  ...
  double cos_angle = vector.dot(vector_proj) / (norm(vector) * norm(
                                                  vector_proj));
  //    maggieDebug2("vector_proj:%s", printP(vector_proj).c_str());
  //    maggieDebug2("cos_angle:%f", cos_angle);
...
static inline Vector3 mult_matrix_vector(const Mat1f & M, const Vector3 & V) {
  ...
  V_matrix(1, 0) = V.y;
  V_matrix(2, 0) = V.z;
  //maggieDebug2("size:%i x %i", V_matrix.cols, V_matrix.rows);
  //V_matrix.t();
  // make the transfo
...
static inline double absolute_angle_between_two_vectors(const Vec2 & v1,
                                                        const Vec2 & v2) {
  //double cos_angle =
  //maggiePrint("cos_angle:%f", cos_angle);
...
static inline double oriented_average_angle_in_vector(const std::vector<Point2> & pts,
  ...
  if ((!to_left && pt_pos >= pts.size()-1) ||
      (to_left && pt_pos <= 0)) {
    maggieDebug3("size:%i, pt_pos:%i and going to %s: "
                 "extreme point! Average angle with its neighbours impossible.",
                 pts.size(), pt_pos, (to_left ? "left" : "right"));
    return 0;
  }
  ...
  maggieDebug3("window_size:%i", window_size);
...
static inline void align_polygons(const std::vector<Point2> & A,
  ...
  double angle_A = geometry_utils::oriented_angle_of_a_vector
                   (A.back() - A.front());
  double angle_B = geometry_utils::oriented_angle_of_a_vector
                   (B.back() - B.front());
...
inline float distance_patterns(const std::vector<_Pt2> & A,
  ...
  //sum_dist += geometry_utils::distance_point_polygon(A[ptA_idx], B_rot, false);
  return sum_dist;
...
convert_coordinates(const _Bbox_src & bbox_src,
  ...
  //    maggieDebug2("bbox:%f+%f -> %i+%i, diff:%f",
...
static inline double find_closest_points_brute_force(
    ...
    combinatorics_utils::Permutation & best_permutation) {
  ...
  combinatorics_utils::Permutation current_permutation;
  combinatorics_utils::create_listing(current_permutation, set1.size());
  typename std::vector<Point2>::const_iterator set1_it;
  combinatorics_utils::Permutation::const_iterator perm_it;
  double current_distance_squared;

  bool next_was_found = true;
  while (next_was_found) {
    //maggiePrint("current_permutation:'%s'",
    ...
    next_was_found = combinatorics_utils::
